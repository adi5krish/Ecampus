...
from campus_admin.forms import ResultForm,FeeReceiptForm

def index(request):

    url1='<a href="/campus_admin/result"> Result </a>'
    url2='<a href="/campus_admin/fee_receipt"> Fee Receipt </a>'
...
def result(request):
    ...
    length = len(batches)
    # number = len(batches)
    #batches['length'] = range(0,number)
    args = {'batches':batches,'length':length}
